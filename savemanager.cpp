...
quint32 SaveManager::sub(const QByteArray* in, int a, int b)
{
    if (a >= 0x14 || b > 2) {
        return 0;
    }
    ...
    if (!b) {
        b = SaveManager::readUint32(in, 0x10);
    }
    if (b) {
        b -= 1;
    }
    if ((b = b * 0xA0 + 0x20)) {
        b = b + 4 * a + 0x1C;
        return SaveManager::readUint32(in, b);
    }
    return 0;
}

quint32 SaveManager::sub2(const QByteArray* in, int a)
{
    if (a > 2) {
        return 0;
    }
    ...
    if (!a) {
        a = SaveManager::readUint32(in, 0x10);
    }
    if (a) {
        a -= 1;
    }
...
QByteArray SaveManager::genKey(QByteArray* decryptedHead, int index = -1)
{
    ...
    a = a ^ SaveManager::sub(decryptedHead, 2, index);
    Xorshift myRNG(a);
    for (int i = sub2(decryptedHead, 0) & 0xFF; i > 0; --i) {
        myRNG.next();
    }
    QByteArray key;
...
Error::ErrorCode SaveManager::loadFile(QString path)
{
    ...
    QFile head(dir.filePath("head.sw"));
    ...
    // generate keys
    QByteArray key1 = genKey(decryptedHead, 0);
...
void SaveManager::reorderF3()
{
    ...
    Xorshift rng_0x04(CRC32::calcCRC32(this->getSectionData(0x04, true)));
    ...
    f3 = this->getSectionById(0xF3);
    for (int i = 0; i < f3->childCount(); ++i) {
        s = f3->child(i);
        if (!order.contains(s->getId())) {
            f3->removeChild(s);
            f3->addChild(s);
        }
    }
